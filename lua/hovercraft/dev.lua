-- Stolen from https://github.com/ThePrimeagen/harpoon
--
-- Don't include this file, we should manually include it via
-- require('hovercraft.dev').reload();
--
-- A quick mapping can be setup using something like:
-- :nmap <leader>rr :lua require('hovercraft.dev').reload()<CR>
local M = {}

function M.reload()
    require('plenary.reload').reload_module('hovercraft')
end

local log_levels = { 'trace', 'debug', 'info', 'warn', 'error', 'fatal' }
local function set_log_level()
    local log_level = vim.env.HOVERCRAFT_LOG or vim.g.hovercraft_log_level
